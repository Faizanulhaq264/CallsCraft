#ifndef MEETING_PARTICIPANTS_CTRL_EVENT_LISTENER_H
#define MEETING_PARTICIPANTS_CTRL_EVENT_LISTENER_H
...
private:
	void (*onIsHost_)();
	void (*onIsCoHost_)();
	std::map<unsigned int, bool> participants; // Map to store participant IDs and their host status
	
	// Add these variables to store the initial IDs
	unsigned int originalHostId_;
	unsigned int originalClientId_;
	bool initialRolesSet_;  // Flag to track if we've set the original roles

public:
	MeetingParticipantsCtrlEventListener(void (*onIsHost_)(), void (*onIsCoHost_)());

	// Get participants map
	const std::map<unsigned int, bool>& getParticipants() const { return participants; }

	// Add getters for the IDs
	unsigned int getOriginalHostId() const { return originalHostId_; }
	unsigned int getOriginalClientId() const { return originalClientId_; }
	...
	/// \brief Method to scan existing participants
	void scanExistingParticipants();
};

#endif // MEETING_PARTICIPANTS_CTRL_EVENT_LISTENER_H
