			...
			if (err1 != SDKERR_SUCCESS) {
				...
				if (isVideo) {
					...
					if (err != SDKERR_SUCCESS) {
						std::cout << "Error occurredd" << std::endl;
						// Handle error
					}
					else {
...
void onInMeeting(MeetingParticipantsCtrlEventListener* participantsListener) {
	...
	if (m_pMeetingService->GetMeetingStatus() == ZOOM_SDK_NAMESPACE::MEETING_STATUS_INMEETING) {
		...
		// After participants are scanned and IDs are available
		if (participantsListener) {
			unsigned int hostId = participantsListener->getOriginalHostId();
			unsigned int clientId = participantsListener->getOriginalClientId();
			audio_source->setParticipantIds(hostId, clientId);
		}
	}

	//first attempt to start raw recording / sending
	CheckAndStartRawRecording(GetVideoRawData, GetAudioRawData);
...
void JoinMeeting()
{
	...
	MeetingParticipantsCtrlEventListener* participantsListener = new MeetingParticipantsCtrlEventListener(&onIsHost, &onIsCoHost);
	m_pMeetingService->SetEvent(new MeetingServiceEventListener(
		&onMeetingJoined,
		&onMeetingEndsQuitApp,
		[participantsListener]() { onInMeeting(participantsListener); },
	...
	withoutloginParam.userName = "ZOOMBOT";
